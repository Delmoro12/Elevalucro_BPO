...
BEGIN
    ...
    CASE p_occurrence_type
        WHEN 'weekly' THEN
            -- Semanal - usar quantidade especificada ou padrão de 52 (1 ano)
            v_installment_count := COALESCE(p_recurrence_count, 52);
            v_interval_days := 7;
        ...
        WHEN 'biweekly' THEN
            -- Quinzenal - usar quantidade especificada ou padrão de 26 (1 ano)
            v_installment_count := COALESCE(p_recurrence_count, 26);
            v_interval_days := 14;
            v_day_of_week := (p_recurrence_config->>'day_of_week')::INTEGER;
        ...
        WHEN 'monthly' THEN
            -- Mensal - usar quantidade especificada ou padrão de 12 (1 ano)
            v_installment_count := COALESCE(p_recurrence_count, 12);
        ...
        WHEN 'quarterly' THEN
            -- Trimestral - usar quantidade especificada ou padrão de 4 (1 ano)
            v_installment_count := COALESCE(p_recurrence_count, 4);
            v_day_of_month := COALESCE((p_recurrence_config->>'day_of_month')::INTEGER, EXTRACT(DAY FROM v_account.due_date));
        ...
        WHEN 'semiannual' THEN
            -- Semestral - usar quantidade especificada ou padrão de 2 (1 ano)
            v_installment_count := COALESCE(p_recurrence_count, 2);
            v_day_of_month := COALESCE((p_recurrence_config->>'day_of_month')::INTEGER, EXTRACT(DAY FROM v_account.due_date));
