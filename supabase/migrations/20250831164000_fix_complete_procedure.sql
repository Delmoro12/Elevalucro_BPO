-- ======================================
-- Fix Companies Fields and Update Procedure
-- ======================================

-- 1. Add segmento column if it doesn't exist
ALTER TABLE companies 
ADD COLUMN IF NOT EXISTS segmento VARCHAR(100);

COMMENT ON COLUMN companies.segmento IS 'Segmento de mercado da empresa';

-- 2. Fix the create_client_signup procedure to include all missing fields
...
VERSÃO ATÔMICA COM CAMPOS CORRETOS: 
- subscription_plan vem do prospect (não default)
- segmento é incluído na empresa
- email e phone são incluídos
...
DO $$ 
BEGIN 
  RAISE NOTICE 'Fixed companies fields and updated procedure to include all prospect data';
END $$;