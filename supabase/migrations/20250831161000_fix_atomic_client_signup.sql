-- ======================================
-- Fix Atomic Client Signup with Better Error Handling
-- ======================================
...
BEGIN
  ...
  RAISE LOG '✅ Company created with ID: %', v_company_id;
  ...
  -- 8. CRITICAL: Setup onboarding checklist - WITH ERROR HANDLING
  RAISE LOG '📋 Setting up onboarding checklist for company: %', v_company_id;
  
  SELECT setup_company_onboarding_checklist(v_company_id) INTO v_onboarding_result;
  ...
  RAISE LOG '🎉 Client signup COMPLETED successfully - Company: %, Onboarding: % items, Routines: % items', 
    v_company_id, 
...
VERSÃO ATÔMICA: Se qualquer parte falhar, toda a operação é revertida.
...
Retorna: JSONB com IDs criados e contadores de items criados
...
8. Configura onboarding checklist (COM VERIFICAÇÃO)
9. Configura rotinas (COM VERIFICAÇÃO)
10. Retorna resultado em JSONB';
