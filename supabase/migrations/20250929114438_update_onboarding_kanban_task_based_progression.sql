...
SELECT 
  ...
  -- Calcular semana de onboarding baseada em conclusão de tarefas por plano
  CASE 
    WHEN (SELECT COUNT(*) FROM companies_onboarding_checklist coc_check
          WHERE coc_check.company_id = c.id 
          AND coc_check.week = 1 
          AND c.subscription_plan = ANY(coc_check.enabled_plans)
          AND coc_check.is_checked = false) > 0 THEN 'semana_1'
          
    WHEN (SELECT COUNT(*) FROM companies_onboarding_checklist coc_check
          WHERE coc_check.company_id = c.id 
          AND coc_check.week = 2 
          AND c.subscription_plan = ANY(coc_check.enabled_plans)
          AND coc_check.is_checked = false) > 0 THEN 'semana_2'
          
    WHEN (SELECT COUNT(*) FROM companies_onboarding_checklist coc_check
          WHERE coc_check.company_id = c.id 
          AND coc_check.week = 3 
          AND c.subscription_plan = ANY(coc_check.enabled_plans)
          AND coc_check.is_checked = false) > 0 THEN 'semana_3'
          
    WHEN (SELECT COUNT(*) FROM companies_onboarding_checklist coc_check
          WHERE coc_check.company_id = c.id 
          AND coc_check.week = 4 
          AND c.subscription_plan = ANY(coc_check.enabled_plans)
          AND coc_check.is_checked = false) > 0 THEN 'semana_4'
          
    ELSE 'concluido'  -- Todas as tarefas foram completadas
