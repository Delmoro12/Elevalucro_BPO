...
CREATE TABLE IF NOT EXISTS companies_routines (
    ...
    routine_id UUID REFERENCES routines(id) ON DELETE CASCADE, -- Nullable para rotinas customizadas
    ...
    -- Note: Unique constraint removed to allow multiple custom routines per company
    -- Custom routines (routine_id NULL) can have multiple entries per company
);
