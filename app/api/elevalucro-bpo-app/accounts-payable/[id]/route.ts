...
export async function PUT(
  ...
  try {
    console.log(`📝 API BPO-APP: Updating account payable ${params.id}...`)
    ...
    console.log(`✅ BPO-APP: Account payable updated: ${params.id}`)
...
export async function DELETE(
  ...
  try {
    console.log(`🗑️ API BPO-APP: Deleting account payable ${params.id}...`)
    ...
    if (error) {
      console.error('❌ Database error:', error)
      return NextResponse.json(
        { success: false, error: 'Erro ao excluir conta a pagar' },
        { status: 500 }
      )
    }

    console.log(`✅ BPO-APP: Account payable deleted: ${params.id}`)
