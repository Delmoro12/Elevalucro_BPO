...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 API BPO-APP: Getting accounts payable from database...')
    ...
    const { data: accountsPayable, error } = await query
    ...
    console.log(`✅ BPO-APP: Found ${formattedAccounts.length} accounts payable`)
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 API BPO-APP: Creating new account payable...')
    ...
    // Criar conta a pagar na tabela unificada (BPO-APP: sempre não validada inicialmente)
    ...
    console.log(`✅ BPO-APP: Account payable created: ${account.id}`)
