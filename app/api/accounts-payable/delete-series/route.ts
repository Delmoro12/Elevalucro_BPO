...
export async function DELETE(request: NextRequest) {
  try {
    console.log('🗑️ API: Deleting accounts series...')
    ...
    switch (delete_type) {
      ...
      case 'unpaid':
        // Apenas contas não pagas (status != 'paid')
    ...
    if (delete_type === 'future') {
      ...
      console.log('💰 Filtering for unpaid accounts (status != "paid")');
      countQuery = countQuery.neq('status', 'paid');
    }
