...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 API: Getting accounts receivable from database...')
    ...
    const { data: accountsReceivable, error } = await supabase
    ...
    console.log(`✅ Found ${formattedAccounts.length} accounts receivable`)
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 API: Creating new account receivable...')
    ...
    // Criar conta a receber na tabela unificada
    ...
    console.log(`✅ Account receivable created: ${account.id}`)
