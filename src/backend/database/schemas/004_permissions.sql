...
COMMENT ON COLUMN permissions.slug IS 'Identificador único da permissão (formato: module.action)';
COMMENT ON COLUMN permissions.module IS 'Módulo do sistema (documents, users, companies, etc.)';
COMMENT ON COLUMN permissions.category IS 'Categoria da permissão (read, write, admin, special)';
COMMENT ON COLUMN permissions.action IS 'Ação específica (create, read, update, delete, etc.)';
COMMENT ON COLUMN permissions.requires_ownership IS 'Requer ser proprietário do recurso para executar';
COMMENT ON COLUMN permissions.requires_same_company IS 'Requer estar na mesma empresa do recurso';
COMMENT ON COLUMN permissions.depends_on_permission_ids IS 'Array de UUIDs de permissões que são pré-requisitos';