...
Por favor, me informe esses dados para que eu possa finalizar o processamento do documento.`,
  ...
  const handleSaveDocument = async () => {
    ...
    try {
      // Simula salvamento do documento
      console.log('Salvando documento:', { documentInfo, ocrData });
      
      // Simula delay de salvamento
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Aqui você faria a requisição para salvar no banco
      // await api.post('/documents', { documentInfo, ocrData });
      ...
      onClose();
      // Aqui adicionaria ao kanban/lista de documentos
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 space-y-4 sm:space-y-6">
                  <div className="text-center">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3 sm:p-4 text-left space-y-2 mb-4 sm:mb-6">
                      <h4 className="font-medium text-slate-900 dark:text-white text-sm">Resumo:</h4>
                      <div className="text-xs space-y-1 text-slate-600 dark:text-slate-400">
                        <div>📄 Tipo: {ocrData?.tipo?.toUpperCase()}</div>
                        <div>💰 Valor: {ocrData?.valor || 'N/A'}</div>
                        <div>📅 Data: {ocrData?.data || 'N/A'}</div>
                        <div>🏢 Fornecedor: {ocrData?.fornecedor || ocrData?.cliente || 'N/A'}</div>
