...
import { Search, RotateCcw, Plus, Calendar, ChevronDown } from 'lucide-react';

// Tipos genéricos para a tabela
export interface DataTable03Column<T = any> {
  key: string;
  title: string;
  width?: string;
  className?: string;
...
export interface DataTable03Tab {
  key: string;
  label: string;
...
export interface DataTable03Filter {
  key: string;
  label: string;
  placeholder?: string;
...
export interface DataTable03DateFilter {
...
export interface DataTable03Props<T = any> {
  // Dados
  data: T[];
  columns: DataTable03Column<T>[];
  loading?: boolean;
  ...
  filters?: DataTable03Filter[];

  // Filtro de data
  dateFilter?: {
    value: DataTable03DateFilter | null;
    onChange: (filter: DataTable03DateFilter | null) => void;
...
export const DataTable03 = <T extends Record<string, any>>(props: DataTable03Props<T>): JSX.Element => {
  const {
    // Dados
    data,
    columns,
  ...
  const [tempDateFilter, setTempDateFilter] = useState<DataTable03DateFilter | null>(null);
  ...
  // Render de célula
  const renderCellValue = (column: DataTable03Column<T>, record: T, index: number) => {
                ...
                {onSearchChange && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-10 pr-4 py-2 w-48 sm:w-64 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                    />
                  </div>
                )}
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                {createButton && (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-colors bg-emerald-600 text-white hover:bg-emerald-700 text-sm flex-shrink-0"
                  >
