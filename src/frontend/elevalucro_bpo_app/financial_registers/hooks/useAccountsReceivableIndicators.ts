...
import { AccountsReceivableSummary, TabIndicator } from '../types/indicators';
...
export const useAccountsReceivableIndicators = (companyId: string) => {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      ...
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/accounts-receivable/summary?company_id=${companyId}`);
        ...
        console.log('🔍 App Accounts Receivable Summary fetched:', data.data);
