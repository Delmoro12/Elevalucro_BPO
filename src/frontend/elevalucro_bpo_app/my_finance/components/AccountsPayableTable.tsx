...
import { TrendingDown, FileText, Clock, CheckCircle, AlertTriangle, XCircle } from 'lucide-react';
...
import type { AccountPayable, AccountPayableStatusTab } from '../types/accountsPayable';
...
type TabKey = AccountPayableStatusTab;

interface AccountsPayableTableProps {
  companyId: string;
}

export const AccountsPayableTable: React.FC<AccountsPayableTableProps> = ({ companyId }) => {
  const [searchValue, setSearchValue] = useState('');
  const [activeTab, setActiveTab] = useState<TabKey>('open');
  const [dateFilter, setDateFilter] = useState<DataTable03DateFilter | null>(null);
  const [showViewModal, setShowViewModal] = useState(false);
  const [selectedTransaction, setSelectedTransaction] = useState<AccountPayable | null>(null);
  
  // Hook para buscar dados das contas a pagar
  const { accountsPayable, loading, error } = useAccountsPayable(companyId);

  // Calcular contadores para as tabs
  const getTabCounts = (): TabCounts => {
    const today = new Date().toISOString().split('T')[0];
    return accountsPayable.reduce((acc: TabCounts, account: AccountPayable) => {
      acc.all++;
  ...
  const handleRowClick = (transaction: AccountPayable) => {
  ...
  const filteredData = accountsPayable.filter(account => {
    // Filtro por tab
    if (activeTab === 'all') {
  ...
  const columns: DataTable03Column<AccountPayable>[] = [
    {
      key: 'supplier_name',
      title: 'Fornecedor',
      width: '200px',
  ...
  // Remover filtros extras - manter apenas busca e filtro de data
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable03<AccountPayable>
        ...
        // Sem ícone de refresh para liberar espaço
