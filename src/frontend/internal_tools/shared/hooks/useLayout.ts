...
export const useLayoutProvider = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedState = localStorage.getItem('internal-tools-sidebar-collapsed');
      if (savedState !== null) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('internal-tools-sidebar-collapsed', JSON.stringify(sidebarCollapsed));
    }
  }, [sidebarCollapsed]);
