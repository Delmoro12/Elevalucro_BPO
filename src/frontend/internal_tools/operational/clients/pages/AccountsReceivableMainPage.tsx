...
import { AccountsReceivableModalSidebar } from '../components/accountsReceivable/AccountsReceivableModalSidebar';
...
import { accountsReceivableRecurrenceService } from '../services/accountsReceivableRecurrence.service';
import { Trash2, Trash } from 'lucide-react';

interface AccountsReceivableMainPageProps {
  companyId: string;
  companyName: string;
}

export const AccountsReceivableMainPage: React.FC<AccountsReceivableMainPageProps> = ({
  companyId,
  companyName
}) => {
  ...
  } = useAccountsReceivable(companyId);
  ...
  const confirmDelete = async () => {
    ...
    const success = await deleteAccountReceivable(showDeleteConfirm);
    if (success) {
  ...
  const confirmDeleteSeries = async () => {
    ...
    try {
      ...
      fetchAccountsReceivable(); // Refresh data
    } catch (error) {
  ...
  const handleSave = async (data: AccountReceivableFormData): Promise<boolean> => {
    try {
      if (selectedAccount) {
        // Update existing
        const success = await updateAccountReceivable(selectedAccount.id, data);
        return success;
      } else {
        // Create new
  ...
  const handleSeriesEditRequest = (account: AccountReceivable, formData: AccountReceivableFormData) => {
  ...
  const handleSingleEdit = async () => {
    ...
    try {
      const success = await updateAccountReceivable(showSeriesEditConfirm.account.id, showSeriesEditConfirm.formData);
      if (success) {
  ...
  const handleSeriesEdit = async (updateType: 'future' | 'all') => {
    ...
    try {
      ...
      await accountsReceivableRecurrenceService.updateRecurringSeries(
  ...
  const getModalTitle = () => {
    if (!selectedAccount) return 'Nova Conta a Receber';
    return 'Editar Conta a Receber';
  };
  ...
  return (
    <div className="space-y-6">
      <AccountsReceivableTable
        ...
        onRefresh={fetchAccountsReceivable}
        onDeleteSeries={handleDeleteSeries}
      />

      {/* Modal Sidebar */}
      <AccountsReceivableModalSidebar
        isOpen={isModalOpen}
        onClose={() => {
          setIsModalOpen(false);
          setSelectedAccount(null);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 max-w-md w-full">
              ...
              <p className="text-slate-600 dark:text-slate-400 mb-6">
                Tem certeza de que deseja excluir esta conta a receber? Esta ação não pode ser desfeita.
              </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 max-w-md w-full">
              ...
              <p className="text-slate-600 dark:text-slate-400 mb-6">
                ...
                Isso excluirá todas as contas não recebidas desta série (incluindo a conta pai se não estiver recebida). Esta ação não pode ser desfeita.
              </p>
...
export default AccountsReceivableMainPage;