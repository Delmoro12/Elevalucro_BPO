...
import { AccountBalance, accountBalancesApi } from '../services/accountBalances.api';
import { CashMovementsFilters, CashMovementsIndicators, TabIndicator } from '../types/indicators';
...
export const useCashMovementsIndicators = (
  ...
  // Buscar saldos das contas - atualizar quando movimentações mudarem
  useEffect(() => {
    const fetchAccountBalances = async () => {
      ...
      setLoading(true);
      try {
        const balances = await accountBalancesApi.getByCompany(companyId);
        console.log('🔍 Account Balances fetched:', balances);
  ...
  }, [companyId, movements]); // Adicionar movements como dependência
  ...
  const indicators = useMemo((): CashMovementsIndicators => {
    ...
    if (filters.selectedAccountId === 'all' || !filters.selectedAccountId) {
      ...
      console.log('🔍 Total balance calculation:', result.totalBalance);
    } else {
