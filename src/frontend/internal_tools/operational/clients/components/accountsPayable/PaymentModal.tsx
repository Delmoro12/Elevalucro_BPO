...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const hasSufficientBalance = () => {
    ...
    return balance >= formData.paid_amount;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Removido: validação de saldo insuficiente
    // Permite pagar mesmo com saldo negativo
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({ ...prev, paid_amount: numberValue }));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-slate-100 ${
                formData.financial_account_id && !hasSufficientBalance() 
            ...
            >
              ...
              {financialAccounts.map(account => {
                ...
                const hasBalance = accountBalance >= formData.paid_amount;
            ...
            {/* Mostrar aviso de saldo insuficiente */}
            {formData.financial_account_id && !hasSufficientBalance() && (
              <p className="text-xs text-red-600 dark:text-red-400 mt-1 flex items-center gap-1">
                ⚠️ Saldo insuficiente. Disponível: {formatCurrency(getSelectedAccountBalance())}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              required
            />
            {formData.paid_amount !== account?.value && (
