...
interface AccountsPayableModalSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: AccountPayableFormData) => Promise<boolean>;
  onSeriesEditRequest?: (account: AccountPayable, formData: AccountPayableFormData) => void;
  title: string;
...
export const AccountsPayableModalSidebar: React.FC<AccountsPayableModalSidebarProps> = ({
  isOpen,
  onClose,
  onSave,
  onSeriesEditRequest,
  ...
  useEffect(() => {
    if (isOpen && account) {
      ...
      const formDataToSet = {
        ...
        date_of_issue: account.date_of_issue || new Date().toLocaleDateString('pt-BR'),
  ...
  const getFilteredSuppliers = () => {
    return suppliers
      ...
      .filter(s => 
        ...
        (s.cpf && s.cpf.includes(supplierSearchTerm))
      );
  };

  // Get selected supplier name
  const getSelectedSupplierName = () => {
    if (!formData.companies_clients_suppliers_id) return 'Selecione um fornecedor';
    const supplier = suppliers.find(s => s.id === formData.companies_clients_suppliers_id);
    return supplier ? supplier.name : 'Fornecedor não encontrado';
  };

  const selectSupplier = (supplierId: string) => {
    setFormData({ ...formData, companies_clients_suppliers_id: supplierId });
    setSupplierDropdownOpen(false);
    setSupplierSearchTerm('');
  };
      ...
      <div className="absolute right-0 top-0 h-full max-w-3xl w-full bg-white dark:bg-slate-800 shadow-2xl flex flex-col animate-slide-in-right">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            {/* Primeira linha - Fornecedor (span completo) */}
            <div className="relative" ref={supplierDropdownRef}>
              ...
              <button
                type="button"
                onClick={() => !isReadOnly && setSupplierDropdownOpen(!supplierDropdownOpen)}
                disabled={isReadOnly}
              ...
              >
                ...
                <ChevronDown className={`h-4 w-4 transition-transform ${supplierDropdownOpen ? 'rotate-180' : ''}`} />
              </button>

              {/* Dropdown */}
              {supplierDropdownOpen && !isReadOnly && (
                      ...
                      getFilteredSuppliers().map(supplier => (
                        <button
                          ...
                          className={`w-full px-3 py-2 text-left hover:bg-slate-100 dark:hover:bg-slate-600 ${
                            formData.companies_clients_suppliers_id === supplier.id 
            ...
            {/* Linha divisória */}
            {(account?.supplier_email || account?.supplier_document || account?.supplier_phone || account?.supplier_whatsapp || account?.supplier_pix) && (
              <>
                <hr className="border-slate-200 dark:border-slate-700 -mx-2" />
                
                {/* Informações do Fornecedor (somente leitura) */}
                    ...
                    {account?.supplier_document && (
                      <div className="flex items-start gap-2">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400">
                          {account.supplier_type === 'PF' ? 'CPF:' : 'CNPJ:'}
                        </span>
